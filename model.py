...
def iterate_minibatches(x, y, batch_size, seed=0):
    ...
    N = x.shape[0]
    idx = shuffle_indices(N, seed=seed)
...
def train_step(params, opt_state, xb, yb, lr, beta_one, beta_two, eps, step):
    ...
    gradient_loss = softmax_cross_entropy_backward(logits, yb)
    grads = lenet_backward(gradient_loss, cache)
    
    new_params, new_opt_state = {}, {}
    for layer in params:
        new_params[layer], new_opt_state[layer] = {}, {}
        for pname in params[layer]:
            g = grads[layer]['d' + pname]
            m = opt_state[layer][pname]['m']
            v = opt_state[layer][pname]['v']
            p_new, m_new, v_new = adam_step(params[layer][pname], g, m, v,
                                step, lr, beta_one, beta_two, eps)
            new_params[layer][pname] = p_new 
            new_opt_state[layer][pname] = {'m':m_new, 'v':v_new}
...
def train_loop(params, x_train, y_train, num_epochs, batch_size, lr=1e-3, beta_one=0.9, beta_two=0.999, eps=1e-8, seed=0):
    ...
    for layer_name, layer in params.items():
        ...
        for pname, arr in layer.items():
            opt_state[layer_name][pname] = {'m':np.zeros_like(arr), 'v':np.zeros_like(arr)}
